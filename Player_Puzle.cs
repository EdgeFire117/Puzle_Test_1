 void Start()
    ...
    //Блок Работы с меню
    public void BackGround_Change(Transform Canvas_S, int Ground, string Ground_Name, string Ground_ChildName)
    {

        //Для работы с единичной иерархией !Начало
        //на основные элементы
        if (Ground == 0)
        {
            foreach (Transform child in Canvas_S)
            {
                //Если меню совпадает то включаем меню
                if (child.name == Ground_Name)
                {
                    child.transform.gameObject.SetActive(true);
                }
                //Если нет то отключаем
                if (child.name != Ground_Name)
                {
        ...
        //Дочерние обьекты включить только(1 меню)
        if (Ground == 3)
        {
            foreach (Transform child in Canvas_S)
            {
                //Если меню совпадает то включаем меню(Несколько Меню)
                if (child.name == Ground_Name)
                {
                    foreach (Transform MenuChild in child)
                    {
                        //включение дочернего обьекта
                        if (MenuChild.transform.name == Ground_ChildName)
                        {
                            MenuChild.transform.gameObject.SetActive(true);
                        }
                    }
                }
            }
        }
        //Дочерние обьекты Отключить только(1 меню)
        if (Ground == 4)
        {
            foreach (Transform child in Canvas_S)
            {
                ...
                if (child.name == Ground_Name)
                {
                    foreach (Transform MenuChild in child)
                    {
                        //Отключенеие дочернего обьекта
                        if (MenuChild.transform.name == Ground_ChildName)
                        {
                            MenuChild.transform.gameObject.SetActive(false);
                        }
                    }
                }
            }
        }
        //Для работы с 2-ой иерархией !Конец
    }
